...
Workflow is a project management platform similar to Linear where teams create and manage issues, track progress, and collaborate. Issues can be organized in parent-child hierarchies — allowing teams to break down large tasks into sub-issues up to 5 levels deep. A "valid parents" feature helps users pick only legitimate parent issues when restructuring their hierarchy.
...
- Requesting valid parents for a non-existent issue should return a not-found error.
