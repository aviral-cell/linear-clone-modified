...
Workflow is a project management platform similar to Linear where teams create and manage issues, track progress, and collaborate. Each issue has an activity timeline that records lifecycle events — such as status changes, priority updates, reassignments, and edits — providing an audit trail of everything that has happened to an issue.
...
3. Change the issue's title — observe that no new activity entry appears in the timeline.
4. Change the issue's status (e.g., from "Todo" to "In Progress") — observe that an activity entry appears, but the old and new values shown are incorrect or swapped.
5. Assign the issue to a team member, then remove the assignee — observe that the removal is not recorded in the timeline.
6. Update the status to the same value it already has — observe that a duplicate activity entry is created when none should appear.
...
- When any tracked field is changed (status, priority, assignee, title, description, project, parent), the activity entry should correctly show the previous value and the new value.
